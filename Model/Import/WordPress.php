...
class WordPress extends AbstractImport
{
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        if ($result) {
            ...
            if ($isReplace && isset($sourceItems)) {
                ...
                foreach ($updateData as $postId => $postImage) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update($this->_resourceConnection->getTableName('baniwal_blog_post'), ['image' => $postImage], $where);
                }
            ...
            $this->_registry->register('baniwal_import_post_ids_collection', $oldPostIds);
            $this->_registry->register('baniwal_import_post_statistic', $statistics);
            return true;
        } else {
            return false;
        }
    ...
    protected function _importTags($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            $this->_importRelationships($data, $connection, $oldTagIds, 'baniwal_blog_post_tag', 'post_tag');
        }

        $statistics = $this->_getStatistics('tags', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('baniwal_import_tag_statistic', $statistics);
    }
    ...
    protected function _importCategories($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            mysqli_free_result($result);
            $this->_importRelationships($data, $connection, $oldCategoryIds, 'baniwal_blog_post_category', 'category', 1);
        }

        $statistics = $this->_getStatistics('categories', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('baniwal_import_category_statistic', $statistics);
    }
    ...
    protected function _importAuthors($data, $connection)
    {
         ...
         */
        $oldPostIds = $this->_registry->registry('baniwal_import_post_ids_collection');
        $updateData = [];
        ...
        foreach ($updateData as $postId => $authorId) {
            $where = ['post_id = ?' => (int)$postId];
            $this->_resourceConnection->getConnection()
                ->update($this->_resourceConnection->getTableName('baniwal_blog_post'), ['author_id' => $authorId], $where);
        }
        $statistics = $this->_getStatistics('authors', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('baniwal_import_user_statistic', $statistics);
    }
    ...
    protected function _importComments($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
                ...
                if ($comment['is_imported']) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update($this->_resourceConnection
                            ->getTableName('baniwal_blog_comment'), [
            ...
            foreach ($upgradeChildData as $commentId => $commentParentId) {
                ...
                $this->_resourceConnection->getConnection()
                    ->update($this->_resourceConnection
                        ->getTableName('baniwal_blog_comment'), ['reply_id' => $commentParentId], $where);
            }
            foreach ($upgradeParentData as $commentId => $commentHasReply) {
                $where = ['comment_id = ?' => (int)$commentId];
                $this->_resourceConnection->getConnection()
                    ->update($this->_resourceConnection
                        ->getTableName('baniwal_blog_comment'), ['has_reply' => $commentHasReply], $where);
            }
            mysqli_free_result($result);
        }

        $statistics = $this->_getStatistics('comments', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('baniwal_import_comment_statistic', $statistics);
    }
    ...
    private function _updatePosts($post, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('baniwal_blog_post'), [
                'name' => $post['name'],
                'short_description' => $post['short_description'],
                'post_content' => $post['post_content'],
                'url_key' => $post['url_key'],
        ...
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('baniwal_blog_post_category'), $where);
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('baniwal_blog_post_tag'), $where);
    }
    ...
    private function _updateCategories($category, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('baniwal_blog_category'), [
                'name' => $category['name'],
                'url_key' => $category['url_key'],
                'description' => $category['description'],
                'meta_robots' => $category['meta_robots'],
