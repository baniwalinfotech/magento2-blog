    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (!$installer->tableExists('baniwal_blog_post')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_post'))
        ...
        if (!$installer->tableExists('baniwal_blog_tag')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_tag'))
        ...
        if (!$installer->tableExists('baniwal_blog_topic')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_topic'))
        ...
        if (!$installer->tableExists('baniwal_blog_category')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_category'))
        ...
        if (!$installer->tableExists('baniwal_blog_post_tag')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_post_tag'))
                ->addColumn('tag_id', Table::TYPE_INTEGER, null, [
                    'unsigned' => true,
                    'primary' => true,
                    'nullable' => false
                ...
                ->addIndex($installer->getIdxName('baniwal_blog_post_tag', ['post_id']), ['post_id'])
                ->addIndex($installer->getIdxName('baniwal_blog_post_tag', ['tag_id']), ['tag_id'])
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_tag', 'post_id', 'baniwal_blog_post', 'post_id'),
                    'post_id',
                ...
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_tag', 'tag_id', 'baniwal_blog_tag', 'tag_id'),
                    'tag_id',
                    $installer->getTable('baniwal_blog_tag'),
                    'tag_id',
                    Table::ACTION_CASCADE
                )
                ->addIndex(
                    $installer->getIdxName('baniwal_blog_post_tag', ['post_id', 'tag_id'], AdapterInterface::INDEX_TYPE_UNIQUE),
                    ['post_id', 'tag_id'],
        ...
        if (!$installer->tableExists('baniwal_blog_post_topic')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_post_topic'))
                ->addColumn('topic_id', Table::TYPE_INTEGER, null, [
                    'unsigned' => true,
                    'primary' => true,
                    'nullable' => false
                ...
                ->addIndex($installer->getIdxName('baniwal_blog_post_topic', ['post_id']), ['post_id'])
                ->addIndex($installer->getIdxName('baniwal_blog_post_topic', ['topic_id']), ['topic_id'])
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_topic', 'post_id', 'baniwal_blog_post', 'post_id'),
                    'post_id',
                    $installer->getTable('baniwal_blog_post'),
                    'post_id',
                    Table::ACTION_CASCADE
                ...
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_topic', 'topic_id', 'baniwal_blog_topic', 'topic_id'),
                    'topic_id',
                    $installer->getTable('baniwal_blog_topic'),
                    'topic_id',
                    Table::ACTION_CASCADE
                )
                ->addIndex(
                    $installer->getIdxName('baniwal_blog_post_topic', ['post_id', 'topic_id'], AdapterInterface::INDEX_TYPE_UNIQUE),
                    ['post_id', 'topic_id'],
                    ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                )
        ...
        if (!$installer->tableExists('baniwal_blog_post_category')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('baniwal_blog_post_category'))
                ->addColumn('category_id', Table::TYPE_INTEGER, null, [
                    'unsigned' => true,
                    'primary' => true,
                    'nullable' => false
                ...
                ->addColumn('position', Table::TYPE_INTEGER, null, ['nullable' => false, 'default' => '0'], 'Position')
                ->addIndex($installer->getIdxName('baniwal_blog_post_category', ['category_id']), ['category_id'])
                ->addIndex($installer->getIdxName('baniwal_blog_post_category', ['post_id']), ['post_id'])
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_category', 'category_id', 'baniwal_blog_category', 'category_id'),
                    'category_id',
                    $installer->getTable('baniwal_blog_category'),
                    'category_id',
                    Table::ACTION_CASCADE
                )
                ->addForeignKey(
                    $installer->getFkName('baniwal_blog_post_category', 'post_id', 'baniwal_blog_post', 'post_id'),
                    'post_id',
                    $installer->getTable('baniwal_blog_post'),
                    'post_id',
                    Table::ACTION_CASCADE
                ...
                ->addIndex(
                    $installer->getIdxName('baniwal_blog_post_category', ['category_id', 'post_id'], AdapterInterface::INDEX_TYPE_UNIQUE),
                    ['category_id', 'post_id'],
                    ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                )
